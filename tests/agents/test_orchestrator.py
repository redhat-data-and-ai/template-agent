"""Tests for orchestrator agent (client-intake skill)."""
...
def pytest_generate_tests(metafunc):
    ...
    if "eval_case" in metafunc.fixturenames:
        skill_config = load_skill_evals("client-intake")
...
def test_orchestrator_evaluation(
    ...
    """Test orchestrator agent with client-intake skill."""
    # Setup workspace
    workspace = workspace_dir / "orchestrator-workspace" / f"eval-{eval_id}"
    ...
    assert pass_rate >= 0.7, (
        f"Orchestrator failed eval-{eval_id}: "
