"""Tests for publisher subagent (email-formatter skill)."""
...
def save_output(output_dir: Path, output: str):
    ...
    (output_dir / "email.html").write_text(output)
...
async def run_agent_async(agent, prompt: str, eval_id: int, tracer) -> str:
    ...
    config = {"configurable": {"thread_id": f"publisher-test-{eval_id}"}}
...
def pytest_generate_tests(metafunc):
    ...
    if "eval_case" in metafunc.fixturenames:
        skill_config = load_skill_evals("email-formatter")
...
def test_publisher_evaluation(
    ...
    """Test publisher subagent with email-formatter skill."""
    # Setup workspace
    workspace = workspace_dir / "publisher-workspace" / f"eval-{eval_id}"
    output_dir = workspace / "outputs"

    # Create agent
    agent = create_publisher_agent(model)
